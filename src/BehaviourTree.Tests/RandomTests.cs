...
namespace BehaviourTree.Tests
{
    ...
    internal sealed class RandomTests
    {
        ...
        public void WhenRandomValueIsBelowThreshold_CallChildAndReturnChildStatus(BehaviourStatus childStatus)
        {
            ...
            randomProvider.SetNextRandomDouble(0.3);
        ...
        [Test]
        public void WhenRandomValueIsEqualToThreshold_DoNotCallChildAndReturnFailure()
        {
        ...
        [Test]
        public void WhenRandomValueIsAboveThreshold_DoNotCallChildAndReturnFailure()
        {
            var child = new MockBehaviour { ReturnStatus = BehaviourStatus.Succeeded };
            var randomProvider = new MockRandomProvider();
            var sut = new Random<MockContext>(child, 0.5, randomProvider);
        ...
        [TestCase(BehaviourStatus.Succeeded)]
        [TestCase(BehaviourStatus.Failed)]
        public void WhenThresholdIsOne_AlwaysCallChild(BehaviourStatus childStatus)
        {
            var child = new MockBehaviour { ReturnStatus = childStatus };
            var randomProvider = new MockRandomProvider();
            var sut = new Random<MockContext>(child, 1.0, randomProvider);

            randomProvider.SetNextRandomDouble(0.99);
